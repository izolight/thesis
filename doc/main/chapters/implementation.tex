...
\begin{figure}
    \begin{center}
        \begin{tabular}{p{3cm}|p{12cm}}
            \textbf{Name} & \textbf{Description} \\ \hline
            sirupsen/logrus & A highly configurable, structured logger \\
            \hline
            mozilla/pkcs7 & Library for parsing and verifying \gls{PKCS7}/\gls{CMS} and RFC3161 timestamps. As the library only supported crls for checking the revocation data, we added support for ocsp. \\
            \hline
            square/go-jose & Implements most of the \gls{JOSE} Standard like \gls{JWT} and \gls{JWK}. \\
            \hline
            coreos/go-oidc & Used for verifying the \gls{OIDC} ID tokens. \\
        \end{tabular}
